...
### 이용가능한 메모리
Hash Join은 해시테이블을 생성해야하기때문에 메모리를 다른 조인들보다 많이 이용하게 된다. 그렇기때문에 메모리 여유분이 있을 때만 Hash Join을 이용할수 있다.

### 데이터 분포
보통 Join되는 필드가 다른 테이블의 ID일 경우가 많지만 아닐때도 있다. 그리고 그 필드가 고유한 값이 아닐때도 있다. 그런 상황에 Hash Join을 이용하게 된다면 같은 해시 함수에 같은 결과값이 나와버린다. 
